...
credentials = service_account.Credentials.from_service_account_file('gclient.json')
scoped_credentials = credentials.with_scopes(scope)
gclient = gspread.authorize(scoped_credentials)

sheet = gclient.open('Bug Tracker').sheet1
...
def bug_add(text: str):
    sheet.insert_row([bug_id(), str(dt.datetime.now()), "Open", text], bug_len())


def bug_close(b_id: str):
    r = sheet.find(b_id, in_column=1).row
    sheet.update_cell(r, 3, "Closed")


def open_bugs():
    return [sheet.row_values(o) for o in [b.row for b in sheet.findall("Open", in_column=3)]]


sheet.insert_row([bug_id(), str(dt.datetime.now()), "Closed", "Testing"], bug_len())
sheet.insert_row([bug_id(), str(dt.datetime.now()), "Open", "Testing how to make the bot do the Excels"], bug_len())
...
class Moderation(commands.Cog):
    ...
    @commands.has_any_role('Rear Admiral')
